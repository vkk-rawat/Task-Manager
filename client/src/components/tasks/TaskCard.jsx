import { CalendarClock, Edit, MessageSquare, Paperclip, Trash2, UserRound } from 'lucide-react';
...
export const TaskCard = ({ task, canManage, onEdit, onDelete, onOpen }) => (
  <article
    className={cn(
      'rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition hover:shadow-panel dark:border-slate-700 dark:bg-slate-900',
      isPastDue(task.dueDate, task.status) && 'border-rose-200 dark:border-rose-900'
    )}
  >
    <div className="flex items-start justify-between gap-3">
      <button className="min-w-0 text-left" onClick={() => onOpen(task)}>
        <h3 className="line-clamp-2 text-sm font-semibold text-slate-950 dark:text-white">{task.title}</h3>
