...
export const getDashboard = asyncHandler(async (req, res) => {
  ...
  const projectProgress = projects.map((project) => {
    const rows = projectTaskCounts.filter(
      (item) => item._id.project.toString() === project._id.toString(),
    );
  ...
  const weeklyMap = new Map(
    weeklyCompleted.map((item) => [item._id, item.count]),
  );
  ...
  res.json({
    ...
    data: {
      ...
      charts: {
        statusBreakdown: normalizeCounts(statusCounts, [
          ...TASK_STATUS_OPTIONS,
          "Overdue",
        ]),
