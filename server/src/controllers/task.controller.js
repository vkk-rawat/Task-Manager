...
const ensureProjectAndAssignee = async (projectId, assignedToId) => {
  ...
  if (!projectContainsUser(project, assignee._id)) {
    throw new ApiError(
      400,
      "Assigned user must belong to the selected project",
    );
  }
...
const canReadTask = (task, user) => {
  ...
  return (
    sameId(task.assignedTo, user._id) ||
    projectContainsUser(task.project, user._id)
  );
};
...
export const createTask = asyncHandler(async (req, res) => {
  const { project } = await ensureProjectAndAssignee(
    req.body.project,
    req.body.assignedTo,
  );
...
export const getTasks = asyncHandler(async (req, res) => {
  ...
  const { q, status, priority, project, assignedTo, sort, page, limit } =
    req.query;
  const filter = {};
...
export const addTaskComment = asyncHandler(async (req, res) => {
  ...
  emitToProject(task.project._id, "comment:created", {
    taskId: task._id,
    comment,
  });
